...
export default function ActivityList({activities, dispatch} : ActivityListProps) {
  ...
  return (
    <>
        ...
        <h2 className="text-3xl font-bold text-center text-slate-600 mb-6">Comida y actividades</h2>
        {activities.length === 0 ?<p className="text-normal font-bold text-center text-slate-600">No hay actividades registradas aún</p> : 
            ...
            <div key={activity.id} className="px-5 py-10 bg-white mt-5 flex justify-between shadow-xl">
                <div className="space-y-2 relative">   
                    <p className={`absolute -top-8 -left-8 px-10 py-2 text-white uppercase font-bold
                     ${activity.category === 1 ? 'bg-blue-500' : 'bg-orange-500'}`}>{categoryName(+activity.category)}</p>
                    <p className="text-2xl font-bold pt-5">{activity.name}</p>
                    <p className="font-black text-4xl text-blue-600">{activity.calories} {''}<span>Calorias</span></p>
                </div>
